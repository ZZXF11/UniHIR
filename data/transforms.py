...
class MaxLongEdgeMinShortEdgeResize_maxsize(torch.nn.Module):
    """Resize the input image so that its longest side is as close as possible to max_size,
    ensuring that both sides are divisible by a specified stride.

    Args:
        max_size (int): Target size for the longest edge of the image.
    ...
    def forward(self, img, img_num=1):
        ...
        # 检查短边是否满足min_size要求
        if min(new_width, new_height) < self.min_size:
            scale = self.min_size / min(new_width, new_height)
            new_width, new_height = self._apply_scale(new_width, new_height, scale)
        
        # 检查是否超过max_pixels限制
        if new_width * new_height > self.max_pixels / img_num:
            scale = (self.max_pixels / img_num / (new_width * new_height)) ** 0.5
            new_width, new_height = self._apply_scale(new_width, new_height, scale)
        
        # 最后再确保长边不超过max_size（因为divisible可能导致稍微超出）
        if max(new_width, new_height) > self.max_size:
            # 尝试向下取整到最接近的divisible值
            if new_width > new_height:
                new_width = self._make_divisible(self.max_size, self.stride)
                # 保持宽高比
                scale = new_width / width
                new_height = self._make_divisible(round(height * scale), self.stride)
            else:
                new_height = self._make_divisible(self.max_size, self.stride)
                # 保持宽高比
...
class ImageTransform:
    def __init__(
        ...
        image_stride, 
        max_size = False,
        max_pixels=16*16*9*2048,
        image_mean=[0.5, 0.5, 0.5], 
        image_std=[0.5, 0.5, 0.5]
    ):
        self.stride = image_stride
        if max_size:
            self.resize_transform = MaxLongEdgeMinShortEdgeResize_maxsize(
