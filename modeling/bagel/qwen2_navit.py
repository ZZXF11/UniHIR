...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if mode == 'und':
            ...
            packed_query_sequence = packed_query_sequence.to(torch.bfloat16) # 查询进来 整个vae的latent是一个q
        ...
        if past_key_values is not None and past_key_values.key_cache[self.layer_idx] is not None:
            ...
            merged_key_states[packed_query_indexes] = packed_key_states # q 是当前的， kv是之前的
            merged_key_states[packed_key_value_indexes] = past_key_states
            merged_value_states[packed_query_indexes] = packed_value_states
            merged_value_states[packed_key_value_indexes] = past_value_states
            key_values_lens = key_values_lens + query_lens
        ...
        # print(f"Q: {packed_query_states.shape}, K: {merged_key_states.shape}, V: {merged_value_states.shape}")
        packed_attn_output = flash_attn_varlen_func(
            q=packed_query_states,
            k=merged_key_states,
            v=merged_value_states,
...
class Qwen2MoTDecoderLayer(nn.Module):
    ...
    def forward_inference(
        ...
        # print(f"Q: {packed_query_sequence.shape}")
