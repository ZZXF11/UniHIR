...
from .modeling_utils import MLPconnector, TimestepEmbedder, PositionEmbedding, PositionEmbedding_infer
...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.visual_gen:
            ...
            timesteps_expanded = packed_timesteps[has_mse][:, None]
            edited_latent_clean = packed_latent_clean[has_mse] - timesteps_expanded * packed_mse_preds
            output_images = self.extract_image_predictions(edited_latent_clean, has_mse, patchified_vae_latent_shapes)
        ...
        return dict(mse=mse, ce=ce, output_images=output_images)

    
    def extract_image_predictions(self, packed_mse_preds, has_mse, patchified_vae_latent_shapes):
        training_images = []
        pred_idx = 0
        ...
        while i < len(has_mse):
            if has_mse[i]:
                start = i
                while i < len(has_mse) and has_mse[i]:
                    i += 1
                end = i
                ...
                for img_idx, (h, w) in enumerate(patchified_vae_latent_shapes):
                    if h * w == segment_length:
                        img_preds = packed_mse_preds[pred_idx:pred_idx + segment_length]
                        training_images.append({
                            'img_idx': img_idx,
                            'shape': (h, w),
                            'predictions': img_preds.reshape(h, w, -1)
                        })
                        pred_idx += segment_length
                        break
            else:
                i += 1
    ...
    def forward_cache_update_vae(
        ...
        packed_timestep_embeds = self.time_embedder(packed_timesteps)
        packed_latent = self.vae2llm(packed_latent) + packed_timestep_embeds + packed_pos_embed # 变成llm输入的维度
